...
def generate_web_images(image, drawable, filename, res_3840, res_2560, res_1920, res_1600, res_1440, res_1280, res_1080, res_720, res_480, res_360, savedir):
    #Aspect Ratio
    h = float(pdb.gimp_image_height(image))
    w = float(pdb.gimp_image_width(image))
    ASPECT_RATIO = float(w / h)

    #COPYRIGHT NOTICE (jpeg)
    COPYRIGHT = "Copyright Gimle Larpes"
    #JPEG SETTINGS
    jpeg_quality = float(0.45)
    jpeg_smooth = float(0.25)
    #WEBP SETTINGS
...
register(
    ...
    "Generates images for use in websites",
    ...
    (PF_FILE, "savedir", "Export Directory:", os.path.join(os.environ['USERPROFILE'],"Pictures")),
    ],
    [],
    generate_web_images,
    menu="<Image>/Image"
